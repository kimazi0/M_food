...
import { Clock, Phone, User as UserIcon, AlertCircle, ChevronRight, CheckCircle2, PlayCircle, Loader2 } from "lucide-react";
...
const COLUMNS = [
  ...
  { id: "COMPLETED", title: "Completed", emoji: "🏁", color: "text-zinc-500", bg: "bg-zinc-500/10", border: "border-zinc-500/20", btn: "bg-zinc-500" },
];
...
export function KitchenDisplay({ initialOrders }: { initialOrders: any[] }) {
  ...
  const handleMoveOrder = async (orderId: string, currentStatus: string, direction: 1 | -1) => {
    ...
    const nextCol = COLUMNS[idx + direction];
    if (!nextCol) return;
  ...
  return (
    <div className="p-8 h-full flex flex-col space-y-8 relative">
      ...
      {/* Kanban Grid */}
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-6 flex-1 overflow-hidden h-full">
        {COLUMNS.map((col) => {
                      ...
                      <div className="space-y-1.5 mb-5 opacity-80 group-hover:opacity-100 transition-opacity">
                         {order.items.map((item: any, idx: number) => (
                           <div key={idx} className="flex items-center gap-2 text-xs font-bold text-zinc-400 group-hover:text-zinc-300">
                             ...
                             <span className="text-[10px] text-zinc-600 ml-auto">x1</span>
                           </div>
                         ))}
                      </div>
